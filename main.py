...
model = torch.hub.load(r'C:\Users\tmdgjsqor1997\Desktop\ai_in_game\scripts\yolov5-master', 'custom', path=r'C:\Users\tmdgjsqor1997\Desktop\ai_in_game\scripts\best.pt', source='local')
...
with mss() as sct:
    ...
    while(True):
        ...
        df= results.pandas().xyxy[0] ##바운딩 박스의 x,y의 최고최소 크기 
        try:
            xmin = int(df.iloc[0,0]) ##각각의 원소 추출
            ymin = int(df.iloc[0,1])
            xmax = int(df.iloc[0,2])
            ymax = int(df.iloc[0,3])

            head_level = (int(xmin + (xmax - xmin) / 2), int(ymin + (ymax - ymin) / 8)) ##타겟의 머리 위치 좌표
